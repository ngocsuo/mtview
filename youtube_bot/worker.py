...
from proxy_module import ProxyManager, ProxyInstance
...
class ViewBotWorker(threading.Thread):
    ...
    def create_proxy(self) -> bool:
        """Create IPv6 proxy."""
        try:
            self.log(f"[Worker {self.worker_id}] Tạo proxy...")
            ...
            self.log(f"[Worker {self.worker_id}] ✅ Proxy: {endpoint}")
            return True
    ...
    def create_profile(self) -> bool:
        ...
        try:
            ...
            # Get socks5 endpoint from proxy instance
            if "socks5" in self.proxy_instance.endpoints:
                endpoint = self.proxy_instance.endpoints["socks5"]
            else:
                endpoint = next(iter(self.proxy_instance.endpoints.values()), None)
            ...
            # Parse "127.0.0.1:PORT" to Hidemium format "SOCKS5|HOST|PORT"
            host, port = endpoint.split(':', 1)
            proxy_str = f"SOCKS5|{host}|{port}"
            
            self.log(f"[Worker {self.worker_id}] Opening profile with proxy: {proxy_str}")
            
            open_resp = self.hidemium_client.open_profile(self.profile_uuid, proxy=proxy_str)
        ...
        try:
            ...
            if self.proxy_instance:
                try:
                    ...
                    self.log(f"[Worker {self.worker_id}] ✅ Xóa proxy")
                except Exception as e:
